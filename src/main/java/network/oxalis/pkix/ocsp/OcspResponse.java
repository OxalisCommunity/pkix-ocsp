...
class OcspResponse {
    ...
    protected network.oxalis.pkix.ocsp.CertificateStatus parseCertificateStatus(org.bouncycastle.cert.ocsp.CertificateStatus certificateStatus) {
        if (certificateStatus == null)
            return network.oxalis.pkix.ocsp.CertificateStatus.GOOD;
        else if (certificateStatus instanceof RevokedStatus)
            return network.oxalis.pkix.ocsp.CertificateStatus.REVOKED;
        else // if (certificateStatus instanceof UnknownStatus)
            return network.oxalis.pkix.ocsp.CertificateStatus.UNKNOWN;
    }
}
